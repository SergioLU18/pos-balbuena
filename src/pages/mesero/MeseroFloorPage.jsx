...
function BotonBarra({ active, onClick, children }) {
  return (
    <button
      onClick={onClick}
      style={{
        ...
        border: `2.5px solid ${active ? 'var(--jb-pink)' : 'var(--jb-line)'}`,
        background: active ? 'var(--jb-pink)' : '#fff',
        color: active ? '#fff' : 'var(--jb-ink)',
      }}
    >
      {children}
    </button>
    ...
    <div className="h-full flex flex-col" style={{ padding: '24px 32px' }}>
      <div className="flex items-center justify-between flex-shrink-0" style={{ marginBottom: 20, gap: 16 }}>
        ...
        <div className="flex items-center" style={{ gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          <BotonBarra active={moviendo} onClick={() => setMoviendo((v) => !v)}>
